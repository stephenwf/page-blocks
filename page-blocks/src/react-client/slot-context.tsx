...
export function SlotContext(props: SlotContextProps & { options: DirectoryOptions; loader: any }) {
  ...
  const fullContext = mergePageBlocksContext((props.options.context as Record<string, string>) || {}, newContext);
  const { data } = useSlotData(
    { loader: props.loader, options: props.options },
    {
      slots: props.slots,
      context: newContext,
      cacheData,
  ...
  let provider = props.name ? (
    <SlotReactContext.Provider value={fullContext}>
  ...
  if (data) {
    provider = <SlotCache.Provider value={{ value: data, context: fullContext }}>{provider}</SlotCache.Provider>;
  }
