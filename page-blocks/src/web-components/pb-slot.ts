import { currentlyAddingBlock, editingMode, register, SlotEditingClient } from '../client';
...
register(
  ...
  () =>
    class PbSlot extends HTMLElement {
      ...
      /** Create a Material Symbols SVG with fill="currentColor" */
      ...
      render() {
        ...
        if (isNew) {
          // ── Slot not yet saved — offer to customise ──────────────────────
          const $bar = el('div', { part: 'slot-bar' }, {}, [
            el('div', { part: 'slot-label' }, {}, [this.slotName]),
            el('div', { part: 'controls' }, {}, [
              el(
                'button',
                { part: 'control control-primary', title: 'Customise slot' },
                { click: this.customiseSlot },
                ['Customise']
              ),
            ]),
          ])!;
        ...
        const $addBtn = el(
          'button',
          { part: 'control control-primary', title: 'Add block' },
          { click: this.addBlock },
          [this.#svgIcon('M440-440H200v-80h240v-240h80v240h240v80H520v240h-80v-240Z')!, 'Add block']
        )!;

        const $resetBtn = el(
          'button',
          { part: 'control control-delete', title: 'Reset slot' },
          { click: this.resetSlot },
          [
            this.#svgIcon(
        ...
        const $slotBar = el('div', { part: 'slot-bar' }, {}, [
          el('div', { part: 'slot-label' }, {}, [this.slotName]),
          el('div', { part: 'controls' }, {}, [$addBtn, $resetBtn]),
        ])!;
